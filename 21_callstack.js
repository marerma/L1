...
// подсчет размера коллстека
// funcSize = место в коллстеке + кол-во переменных * размер переменной
// размер коллстека без переменных = (место ф-ции в коллстеке + 0 * размер переменной)
// если вызывать рекурсивный подсчет глубины коллстека, например, с 5 переменными, хранящими числа, то в хроме размер будет ~8944
// такми образом можно подсичтать размер коллстека в мб, принимая что числа "весят" 8байт
