...
function convertStringToJSON(string) {
 ...
 // пропустить двоеточие
  ...
  // функция парсинга значения: пропускаем пробел, если есть, далее определяем значение в зависимости от типа
  const parseValue = () => {
    skipSpace();
    const value = 
      parseString() ??
  ...
  const parseArray = () => {
    if (string[pos] === '[') {
      ...
      let firstCicle = true; 
      // при первом элементе нам не нужно очищать/пропускать возможные запятые или пробелы, поэтому добавляем флаг
      // для отслеживания. При след.элементах, мы пропускаем пробел и запятые между элементами, пример: [1, 2, 3]

      // пока не встретим закрывающую квадратную скобку, двигаем указатель (после первого элемента сдвигаем, уже пропуская запятую и пробел)
      while (string[pos] !== ']') {
        if(!firstCicle) {
          skipComma();
          skipSpace();
  ...
  const parseObject = () => {
    if (string[pos] === '{') {
      ...
      let firstCicle = true; // после первого ключа нам не нужно очищать/пропускать возможные запятые или пробелы, поэтому добавляем флаг
  ...
  // далее если последующие символы позволяют составить значение true, false или null, то сдвигаем указатель на длину значение (4 или 5 соответственно) и возвращаем распарснное значение
  ...
  // если массив оказывется не пустой, то преобразуем массив в строку, а ее переводим к числу.
  const parseNumber = () => {
    ...
    let num = [];
    // проверяем является ли символ числом или точкой для чисел с плавающей точкой
  ...
  //запускаем работу функции с первого символа переданного аргумента
  let pos = 0;
  return parseValue();
}
